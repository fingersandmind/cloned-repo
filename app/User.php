...
class User extends Model implements AuthenticatableContract, CanResetPasswordContract
{
   ...
   public static function upadteUserBalance($user_id,$sponsor_commisions){
    ...
    public static function add($data,$sponsor_id,$placement_id){
            ...
            $userresult->default_account =$account_id->id ;
            $userresult->save();
           ...
           // dd($placement_id);
            ...
            // $placement_id = UserAccounts::where('user_id',$placement_id)->where('matrix',$data['package'])->first()->id ;
            ...
            $sponsor_account = UserAccounts::where('user_id',$sponsor_id)->where('matrix',$data['package'])->value('id');
    ...
    public static function updateinfo($user_details){

                    $placement_id =  $sponsor_id  = User::where('username',$user_details->sponsor_id)->first()->id ;

                    $uniqid = uniqid();
                    $data['firstname'] = $user_details->user_detail_name ;
                    $data['lastname'] =  $user_details->user_detail_second_name ;
                    ...
                    $data['phone'] = $user_details->user_detail_mobile ;
                    $data['gender'] = $user_details->user_detail_gender ;
                    $data['country'] = 'IN' ;
                    $data['state'] = $user_details->user_detail_state ;
                    $data['city'] = $user_details->user_detail_city ;
                    $data['address'] = $user_details->user_detail_address .' ' .$user_details->user_detail_address2 ;
                    $data['zip'] = '987654' ;
                    $data['location'] = '13325' ;
                    $data['package'] = '1' ;  
                    ...
                    $sponsorvaccant = Sponsortree::createVaccant($sponsor_id, $sponsortree->position);
                     ...
                     $uservaccant = Sponsortree::createVaccant($userresult->id, 0);
                       Transactions::sponsorcommission($sponsor_id,$account_id->id,1);  
    }
