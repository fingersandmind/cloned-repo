          ...
          return false ;
            ...
            if ($root) {
             $data= self::where('sponsortree.user_id', $sponsor)
                         ...
                         ->leftJoin('user_accounts','sponsortree.user_id','=','user_accounts.user_id')
             ...
             $data= self::where('sponsortree.sponsor',$sponsor)
                        ...
                        ->leftJoin('point_table','sponsortree.user_id','=','point_table.user_id')
                        ->leftjoin('profile_infos','profile_infos.user_id','=','sponsortree.user_id')
                        ->select('sponsortree.*','users.username','profile_infos.image','user_accounts.matrix as package')
                        ->get();           
            }
                ...
                foreach ($data as $value) {
                      if($value->type =="yes" || $value->type =="no" ){  
                        ...
                        $downline_count = SELF::leftJoin('users','sponsortree.user_id','=','users.id')
                                                ->where('sponsor',$value->user_id)
                                                ->where('users.confirmed','=','1')
                                                ->where('type','!=','vaccant')
                                                ->count() ;
                        ...
                        if($downline_count >0  &&  $level == 2){
                            $content .=  '<div class="row" style="height: 0px;"><i class="fa fa-angle-double-down" style="font-size:36px"></i></div>' ;
                        }
