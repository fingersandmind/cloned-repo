...
class LeadershipBonus extends Model
{
    ...
    public static function allocateCommission($from_id,$user_id,$package,$level=1){


        $settings = Settings::getSettings();

        $level_one = $settings[0]->indirectfast_track_one;
        $level_two = $settings[0]->indirectfast_track_two;
        $level_three = $settings[0]->indirectfast_track_three;


        $level_bonus = [1 =>$level_one,2=>$level_two,3=>$level_three];

              $amount = Packages::where('id',$package)->value('amount');
              $column = 'level_'.$level ;
            ...
            // dd($bonus_percent );
             if($bonus_percent){
                ...
                if($level <= 2 && $sponsor_id > 0){
                    return self::allocateCommission($from_id,$sponsor_id,$package,++$level);
                }
            }
            return ;
        }
