    ...
    $(function() {
        ...
        if ($('#btn-restart-genealogy-node').length) {
            $('#btn-restart-genealogy-node').on('click', function() {
                ...
                var plan = $('#plan').val();
                options.data = 'getTree/'+levellimit+"/"+plan;
                oc.init(options);
                new PNotify({
                    text: 'Resetting tree',
                    delay: 1000,
       ...
       // Load sponsor tree init ******************************
        ...
        $('#treediv').on('click', '.orgchart .node .title', function() {
            var plan = $('#plan').val();
            options.data = 'getChildrenGenealogy/' + $(this).parent('.node').find('.content img').data('accessid')+'/'+levellimit+"/"+plan;
        ...
        // ---------------------------sponsor tree iterate starts-------
         ...
         $('#sponsortreediv').on('click', '.orgchart .node .title', function() {
          ...
          $('#sponsortreediv').on('click', '.orgchart .node .content', function() {
            options.data = 'sponsor-child/' + $(this).parent('.node').find('.content img').data('accessid');
            options.createNode = function($node, data) {
                var secondMenuIcon = $('<i>', {
                    'class': 'fa fa-info-circle second-menu-icon show-pop',
...
function users_join(data) { 
    ...
    if(CLOUDMLMSOFTWARE.admin){
            ...
            var dom = document.getElementById("joinings_graph");        
    }
...
$( "#switchaccount" ).change(function() {
    $.ajax({
                url: (CLOUDMLMSOFTWARE.admin == true) ? CLOUDMLMSOFTWARE.siteUrl + '/admin/switchaccount' : CLOUDMLMSOFTWARE.siteUrl + '/user/switchaccount'  ,
                type: "post",
                async: false,
                dataType: "json",
                data: {
                ...
                success: function(data, textStatus, jqXHR) {
                    ...
                    window.location.href = CLOUDMLMSOFTWARE.currentUrl ;
